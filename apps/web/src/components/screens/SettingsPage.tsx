...
import { goToStage } from "../../lib/lesson-config";
  ...
  return (
    <motion.div className="settings-stage" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      <main className="settings-shell">
        <motion.section className="settings-card" initial={{ y: 20, opacity: 0, scale: .985 }} animate={{ y: 0, opacity: 1, scale: 1 }} transition={{ duration: .42, ease: [0.22, 1, 0.36, 1] }}>
          <div className="settings-head">
            <button type="button" aria-label="Back home" onClick={() => goToStage("course")}>←</button>
          ...
          <button type="button" className="settings-primary" onClick={() => goToStage("course")}>Done</button>
        </motion.section>
      </main>
    </motion.div>
  );
